...
VisualEvent.prototype = {
	...
	"_construct": function ()
	{
		...
		/**/
		for(var i = 0; i < this.s.elements.length; i++)
		{
			var accumulator = "";
			var eventType = "";
			for(var j = 0 ; j < this.s.elements[i].listeners.length; j++)
			{
				//alert("j: " + j + " -- " + this.s.elements[i].listeners[j].type);
				if(j > 0)
				{
					accumulator += ", ";
				}
				accumulator += this.s.elements[i].listeners[j].type;
				eventType = this.s.elements[i].listeners[j].type;
			}

			/**debug**
			var ihtml = (i + "-1: " + this.s.elements[i]) + "<br><br>";
			ihtml += (i + "-2: " + this.s.elements[i].node) + "<br><br>";
			ihtml += (i + "-3: " + this.s.elements[i].node.offsetLeft) + "<br><br>";
			/**end debug**/

			var theDataStuff = "";
			/** taking the metadata from original interaction script **/
			var xPosition = (this.s.elements[i].node.offsetLeft - this.s.elements[i].node.scrollLeft +this.s.elements[i].node.clientLeft);
			var yPosition = (this.s.elements[i].node.offsetTop - this.s.elements[i].node.scrollTop + this.s.elements[i].node.clientTop);
			var ID = (this.s.elements[i].node.id);

			var thePushed = (this.s.elements[i].node.outerHTML).replace(/(\r\n|\n|\r)/gm,"");
			if(thePushed.indexOf("<select") > -1 && thePushed.indexOf("<option") > -1 && (thePushed.indexOf("<body") < 0))
			{
				//alert("pushed: " + thePushed);
				var ddlArray= new Array();
				theDataStuff += (eventType + ", " + ID + ", "+ "[" + xPosition + "," + yPosition + "]" + ", ");
				for (k = 0; k < this.s.elements[i].node.options.length; k++) {
					ddlArray[k] = this.s.elements[i].node.options[k].value;
					theDataStuff += "value:" + this.s.elements[i].node.options[k].value + ", ";
				}
				theDataStuff += ("<!-- [innerHTML] " + escape(this.s.elements[i].node.innerHTML) + " -->");
			}
			else
			{
				if(eventType != "load")
				{
					theDataStuff += (eventType + ", " 
						+ ID + ", "
						+ "[" + xPosition + "," + yPosition + "]" + ", "
						+ "<!-- [innerHTML] " + escape(this.s.elements[i].node.innerHTML) + " -->\n");
				}
			}
			/** end metadata from original interaction script **/



		//	alert("constructing...2." + i + ": " + accumulator + " - " + $(this.s.elements[i].node));
			if(eventType != "load")
			{
				sneakyDiv += i + ": " + theDataStuff + "\n\n";
			}
		}
		sneakyDiv += "\n\n\n";

		var n = document.createElement( 'div' );
		n.setAttribute( 'id', 'jbrunellesSuperSneakyDiv' );
		n.setAttribute( 'hidden', 'true' );
		document.body.appendChild( n );


		document.getElementById("jbrunellesSuperSneakyDiv").innerHTML = sneakyDiv;


		/**debug**		
		var n = document.createElement( 'div' );
		n.setAttribute( 'id', 'debugDiv' );
		n.setAttribute( 'hidden', 'false' );
		document.body.appendChild( n );

		document.getElementById("debugDiv").innerHTML = ihtml;
	...
	{
		/**jbrunelle commenting to prevent viz layer **
		var that = this,
			events = 0, i, iLen;
